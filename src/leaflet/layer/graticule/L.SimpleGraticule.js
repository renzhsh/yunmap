...
L.SimpleGraticule = L.LayerGroup.extend({
        ...
        if (!this.options.hidden) {
            ...
            for (var i = 0; i < this.options.zoomIntervals.length; i++) {
                if (
                    ...
                    this.options.interval = this.options.zoomIntervals[
                        i
                    ].interval;
                    break;
                }
    ...
    getLineCounts: function() {
        return {
            x: Math.ceil(
                (this._bounds.getEast() - this._bounds.getWest()) /
                    this.options.interval
            ),
            y: Math.ceil(
                (this._bounds.getNorth() - this._bounds.getSouth()) /
                    this.options.interval
            )
        };
    },
    ...
    buildLabel: function(axis, val) {
        ...
        var strval = val.toFixed(1).replace(".0", "");
        ...
        return L.marker(latLng, {
            ...
            icon: L.divIcon({
                ...
                html: '<div class="' + axis + '">' + strval + "</div>"
            })
        });
    },
...
L.simpleGraticule = function(options) {
    let defs = {
        interval: 10,
        zoomIntervals: [
            { start: 0, end: 2, interval: 60 },
            { start: 3, end: 4, interval: 30 },
            { start: 5, end: 6, interval: 10 },
            { start: 7, end: 8, interval: 5 },
    ...
    return new L.SimpleGraticule(Object.assign(defs, options));
};
